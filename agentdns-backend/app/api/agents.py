...
def get_agent_monitoring(
    ...
    usage_records = db.query(
        ...
        AgentUsage.agent_id == agent_id,
        AgentUsage.requested_at >= start_date,
        AgentUsage.requested_at <= end_date
    ).group_by(func.date(AgentUsage.requested_at)).order_by(func.date(AgentUsage.requested_at)).all()
