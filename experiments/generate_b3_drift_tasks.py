...
OUT = ROOT / "experiments" / "outputs" / "b3_drift_120tasks.json"
...
def prefix(idx: int) -> str:
    return "b3p1" if idx <= 14 else ("b3p2" if idx <= 27 else "b3p3")

tasks = []
for i in range(40):
    idx = i + 1
    tasks.append({"task_id": f"{prefix(idx)}_translation_{idx:03d}", "scenario_family":"b3_drift", "complexity_level":"L1", "task_goal":"翻译任务", "task_prompt": translation_prompts[i % len(translation_prompts)], "required_service_categories":["translation"]})
for i in range(40):
    idx = i + 1
    tasks.append({"task_id": f"{prefix(idx)}_summary_{idx:03d}", "scenario_family":"b3_drift", "complexity_level":"L1", "task_goal":"文本摘要", "task_prompt": summary_prompts[i % len(summary_prompts)], "required_service_categories":["text_summary"]})
for i in range(40):
    idx = i + 1
    tasks.append({"task_id": f"{prefix(idx)}_extract_{idx:03d}", "scenario_family":"b3_drift", "complexity_level":"L1", "task_goal":"结构化抽取", "task_prompt": extract_prompts[i % len(extract_prompts)], "required_service_categories":["structured_extraction"]})
