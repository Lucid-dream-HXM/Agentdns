...
def main() -> None:
    ...
    probe = embedding_service.create_query_embedding("b2 hard market vector probe")
    embed_dim = len(probe)

    schema_dim = None
    for field in coll.schema.fields:
        if field.name == 'embedding':
            schema_dim = int(field.params.get('dim'))
            break
    ...
    services = db.query(Service).filter(
        Service.agentdns_uri.like('%experiment/market/%')
    ).all()
    ...
    for svc in services:
        ...
        rows.append({
            ...
            "organization_id": int(svc.organization_id) if svc.organization_id else 1,
            "embedding": vec,
        })
        service_ids.append(int(svc.id))

    if service_ids:
        expr = f"service_id in {service_ids}"
        try:
            coll.delete(expr)
        except Exception:
            pass
